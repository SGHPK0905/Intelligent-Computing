...
\newpage
\phantomsection
\section*{BẢNG PHÂN CÔNG NHIỆM VỤ VÀ MỨC ĐỘ ĐÓNG GÓP}
\addcontentsline{toc}{section}{Bảng phân công nhiệm vụ}

Trong suốt quá trình thực hiện đồ án, các thành viên trong nhóm đã phối hợp chặt chẽ để hoàn thành mục tiêu đề ra. Dưới đây là chi tiết phân công công việc và đánh giá mức độ đóng góp dựa trên tổng khối lượng thực hiện của dự án:

\begin{table}[H]
    \centering
    \renewcommand{\arraystretch}{1.6}
    \resizebox{\textwidth}{!}{
    \begin{tabular}{|c|l|c|p{8.5cm}|c|}
        \hline
        \textbf{STT} & \textbf{Họ và tên} & \textbf{MSSV} & \textbf{Nhiệm vụ phân công} & \textbf{Đóng góp} \\
        \hline
        1 & Nguyễn Trương Cao Sơn & 3123580040 & Đảm nhiệm lập trình thuật toán logic mờ cốt lõi, phát triển giao diện mô phỏng tương tác (Streamlit). Biên soạn chuyên sâu và trình bày nội dung thiết kế, mô phỏng hệ thống (Chương 3, 4). & 35\% \\
        \hline
        2 & Phan Đồng Minh Quân & 3123580038 & Xây dựng, thực thi các kịch bản kiểm thử (Test Cases) và kiểm lỗi (Debug) mã nguồn Web App. Nghiên cứu tổng quan, biên soạn và trình bày nền tảng cơ sở lý thuyết (Chương 1, 2). & 35\% \\
        \hline
        3 & Võ Minh Tiến & 3123580053 & Khảo sát tham số vật lý nguyên mẫu máy giặt LG. Hỗ trợ kiểm thử hệ thống, thiết kế Slide thuyết trình. Biên soạn hướng phát triển (Chương 5, Phụ lục) và chuẩn hóa định dạng toàn bộ báo cáo. & 30\% \\
        \hline
        \multicolumn{4}{|c|}{\textbf{TỔNG CỘNG}} & \textbf{100\%} \\
        \hline
    \end{tabular}
    }
\end{table}

\textbf{Đánh giá chung:} Toàn nhóm đã nỗ lực hoàn thành 100\% các hạng mục đề tài đúng thời hạn, đảm bảo tính nhất quán giữa mô hình toán học và chương trình mô phỏng.
...
spin_speed['Rat_cao'] = fuzz.trimf(spin_speed.universe, [1150, 1350, 1400])
...
Bảng \ref{tab:versions} liệt kê chi tiết các phiên bản thư viện được sử dụng để đảm bảo tính ổn định và khả năng tái lập của thực nghiệm.

\begin{table}[H]
    \centering
    \caption{Danh mục phiên bản các thư viện phát triển}
    \label{tab:versions}
    \begin{tabular}{|l|c|l|}
        \hline
        \textbf{Thư viện / Môi trường} & \textbf{Phiên bản} & \textbf{Vai trò cốt lõi} \\
        \hline
        Python & 3.11.x & Môi trường thực thi chính \\
        \hline
        scikit-fuzzy & 0.4.2 & Engine suy diễn mờ Mamdani \\
        \hline
        NumPy & 1.26.4 & Xử lý ma trận dữ liệu và không gian nền \\
        \hline
        Streamlit & 1.32.0 & Kiến trúc giao diện người dùng tương tác \\
        \hline
        Plotly & 5.19.0 & Kết xuất không gian điều khiển 3D \\
        \hline
    \end{tabular}
\end{table}

\textbf{Đánh giá hiệu năng và độ trễ tính toán:}\\
Một trong những rào cản lớn nhất khi đưa Trí tuệ nhân tạo xuống thiết bị gia dụng là giới hạn về phần cứng. Do đó, mã nguồn Python được nhóm nghiên cứu thiết kế tối ưu hóa ma trận tính toán bằng thư viện NumPy. Quá trình kiểm thử thực nghiệm trên CPU tiêu chuẩn cho thấy:
\begin{itemize}
    \item \textbf{Thời gian khởi tạo không gian:} $\sim 15$ mili-giây (chỉ chạy 1 lần khi bật máy).
    \item \textbf{Độ trễ suy diễn:} $\sim 4-6$ mili-giây cho một chu kỳ quét toàn bộ 19 luật và giải mờ Trọng tâm đa biến.
\end{itemize}
Với độ trễ ở mức mili-giây, bộ điều khiển hoàn toàn đáp ứng được chuẩn thời gian thực. Nếu được biên dịch sang ngôn ngữ C/C++ để nạp vào các vi điều khiển nhúng giá rẻ (như STM32 hay ESP32) trên board mạch máy giặt thực tế, hệ thống vẫn đảm bảo dư thừa tài nguyên xử lý mà không gây tình trạng nghẽn cổ chai).

\subsection{Giao diện người dùng và Trực quan hóa kết quả}
Ứng dụng Web (\textit{Web App}) không đơn thuần là một giao diện tương tác, mà đóng vai trò là một \textbf{Môi trường giả lập và Kiểm chứng thuật toán}. Mỗi Tab chức năng được thiết kế để mở hộp đen của hệ suy diễn mờ, cho phép nhóm nghiên cứu khai thác dữ liệu từ nhiều góc độ.
...
\subsubsection{Hiển thị kết quả đầu ra và các Tab phân tích chuyên môn}
Ngay khi dữ liệu đầu vào có sự biến thiên, hệ thống lập tức thực thi engine suy diễn mờ và cập nhật các lệnh điều khiển. Để phục vụ cho mục đích nghiên cứu và chẩn đoán thuật toán, các thông số đầu ra được nhóm bóc tách và trực quan hóa qua 4 phân hệ chức năng chuyên sâu:
...
\subsection{Kết quả thực nghiệm và Phân tích chuyên sâu}
Để đánh giá toàn diện năng lực của bộ điều khiển, hệ thống đã được kiểm thử với 5 kịch bản đặc thù trích xuất từ mã nguồn. Bảng \ref{tab:test_results} tổng hợp các kết quả giải mờ thực tế.

\begin{table}[H]
    \centering
    \caption{Kết quả nội suy từ bộ điều khiển mờ cho 5 kịch bản tiêu chuẩn}
    ...
    \begin{tabular}{|l|c|c|c|c|c|}
        ...
        \hline
        \textbf{Thời gian (phút)} & $130.6$ & $38.1$ & $80.0$ & $82.5$ & $91.7$ \\
        \textbf{Vắt (RPM)}        & $1165$  & $452$  & $883$  & $631$  & $986$  \\
        \textbf{Nhiệt độ ($^\circ$C)} & $82.9$  & $21.7$ & $46.7$ & $45.4$ & $32.0$ \\
        \textbf{Nước (\%)}        & $85.3$  & $14.7$ & $50.0$ & $45.8$ & $86.0$ \\
        \textbf{Xà phòng (\%)}    & $86.1$  & $14.7$ & $50.0$ & $45.8$ & $65.0$ \\
        \hline
    \end{tabular}
    }
\end{table}

\subsubsection{Phân tích kỹ thuật theo từng kịch bản}
\begin{itemize}
    \item \textbf{TC1 (Heavy Duty):} Mặc dù độ bẩn và khối lượng ở mức cực đại, hệ thống chỉ xuất ra tốc độ vắt $1165$ RPM ($\sim 83\%$ công suất). Điều này chứng minh thuật toán đã tính toán điểm cân bằng để bảo vệ động cơ khỏi rung lắc quá tải trong khi vẫn duy trì nhiệt độ cao ($82.9^\circ$C) để phân rã dầu mỡ.
    \item \textbf{TC2 (Vải lụa):} Kết quả $452$ RPM minh chứng Luật ghi đè an toàn (Rule 9) đã kích hoạt thành công. Nhiệt độ $21.7^\circ$C (nước lạnh) được duy trì để bảo vệ màu sắc và cấu trúc sợi tơ lụa nhạy cảm.
    \item \textbf{TC3 (Đồ hỗn hợp):} Với các thông số đầu vào đều ở mức trung bình ($50\%$), kết quả đầu ra cũng đạt trạng thái cân bằng tuyệt đối ($80.0$ phút, $50\%$ nước). Đây là "điểm neo" khẳng định tính chính xác của ma trận luật cơ sở.
    \item \textbf{TC4 (Đồ trẻ em):} Một tình huống xung đột điển hình: Đồ rất bẩn ($85\%$) nhưng vải rất nhạy cảm (Điểm 8). Hệ thống đã xử lý thông minh bằng cách cho thời gian giặt dài ($82.5$ phút) để làm sạch sâu nhưng giữ tốc độ vắt thấp ($631$ RPM) để tránh xé rách sợi vải mỏng.
    \item \textbf{TC5 (Chăn mền):} Lượng nước ($86\%$) cao hơn đáng kể so với Xà phòng ($65\%$). Logic này phản ánh đúng thực tế: tải trọng $9$ kg cần nhiều nước để ngấm đều chăn lớn, nhưng do độ bẩn chỉ $40\%$ nên hệ thống tự động giảm xà phòng để tiết kiệm và tránh tồn dư hóa chất.
\end{itemize}

\subsubsection{Đối chiếu hiệu quả với Logic Boolean (Ngưỡng cứng)}
Bảng \ref{tab:comparison} trình bày ưu thế định lượng của bộ điều khiển mờ so với các dòng máy giặt truyền thống sử dụng logic ngưỡng cứng.

\begin{table}[H]
    \centering
    \caption{Ước tính hiệu quả của Logic mờ so với Logic Boolean}
    \label{tab:comparison}
    \resizebox{\textwidth}{!}{
    \renewcommand{\arraystretch}{1.3}
    \begin{tabular}{|l|c|c|l|}
        \hline
        \textbf{Thông số} & \textbf{Logic Boolean (Ước tính)} & \textbf{Bộ điều khiển Fuzzy} & \textbf{Hiệu quả đạt được} \\
        \hline
        Thời gian (TC1) & $180$ phút (Cố định) & $130.6$ phút & Tiết kiệm $\sim27\%$ thời gian \\
        Nước (TC5)      & $100\%$ (Ngưỡng tải nặng) & $86\%$ & Tiết kiệm $14\%$ nước cấp \\
        Tốc độ (TC2)    & $800$ RPM (Mặc định) & $452$ RPM & Bảo vệ sợi vải tốt hơn gấp 2 lần \\
        \hline
    \end{tabular}
    }
\end{table}
...
Dựa trên việc quét lưới $2500$ điểm dữ liệu, nhóm trích dẫn các dải giá trị thực tế để minh chứng cho tính ổn định:
\begin{itemize}
    \item Tại kịch bản Tải nặng (TH1), tốc độ vắt biến thiên liên tục trong dải $[800, 1165]$ RPM tùy theo độ nhạy vải. 
    \item Tại kịch bản Vải mỏng (TH2), toàn bộ mặt cong bị ép xuống dải an toàn $[300, 550]$ RPM, cho thấy sự can thiệp tuyệt đối của luật bảo vệ vật liệu.
    \item \textbf{Độ dốc mặt cong:} Quan sát vùng \texttt{cloth\_sensitivity} $\in [4, 6]$, mặt cong không xuất hiện hiện tượng đứt gãy hay nhảy bậc (Gradient thay đổi mượt mà). Điều này khẳng định hệ thống giải mờ Trọng tâm giúp máy vận hành êm ái, tránh các xung lực đột ngột cho động cơ — một nhược điểm chí mạng của logic nhị phân truyền thống.
\end{itemize}
...
Sự khác biệt về cao độ và địa hình giữa Hình \ref{fig:3d_heavy} và Hình \ref{fig:3d_delicate} minh chứng rõ nét cho sự tương tác phi tuyến của ma trận 19 luật mờ:

\begin{itemize}
    \item \textbf{Thuộc tính Ghi đè:} Ở kịch bản \textit{Vải mỏng} (Hình \ref{fig:3d_delicate}), trục "Độ nhạy vải" hoàn toàn chiếm quyền kiểm soát. Bất chấp sự gia tăng của trục "Mức dầu mỡ", toàn bộ mặt cong bị hệ thống "ép phẳng" ở dải cao độ thấp ($< 500$ RPM). Điều này khẳng định Lớp luật an toàn (Rule 9) đã hoạt động xuất sắc, chặn đứng mọi rủi ro xé rách sợi vải.
    \item \textbf{Độ dốc chuyển mạch:} Ở kịch bản \textit{Đồ bảo hộ} (Hình \ref{fig:3d_heavy}), mặt cong duy trì trần vận tốc ly tâm rất cao (đỉnh màu vàng) để vắt kiệt nước cho tải nặng. Tuy nhiên, thay vì rớt thẳng đứng dạng bậc thang như logic Boolean, mặt cong đổ dốc cực kỳ mềm mại khi di chuyển về phía các loại vải mỏng. 
    \item \textbf{Tính trơn nhẵn của không gian:} Việc không xuất hiện bất kỳ "hố sụt" (\textit{Fuzzy holes}) hay "đỉnh gai" đột biến nào trên cả hai bề mặt đã khẳng định kỹ thuật thiết lập giao thoa $25-50\%$ giữa các hàm \textit{Trimf} là hoàn toàn chuẩn xác. Động cơ thực tế khi chạy theo dải thông số nội suy này sẽ tăng giảm gia tốc cực kỳ êm ái, bảo vệ tối đa hệ thống truyền động cơ khí.
\end{itemize}
% ===================================================================
